...
function articleTerms(a: Article): Set<string> {
  const stop = new Set(["the","and","for","with","from","that","this","says","said","will","into","after","over","under","about","have","has","was","were","are","its"]);
  return new Set(
    `${a.title} ${a.excerpt}`
      .toLowerCase()
      .replace(/[^a-z0-9\s]/g, " ")
...
function titleTerms(a: Article): Set<string> {
  const stop = new Set(["the","and","for","with","from","that","this","says","said","will","into","after","over","under","about","have","has","was","were","are","its","some"]);
  return new Set(
    a.title
      .toLowerCase()
      .replace(/[^a-z0-9\s]/g, " ")
...
export async function runQuick(input: QuickInput): Promise<{ report: Quick; meta: Omit<FinalMeta, "scope" | "perTarget" | "toppedUp"> & { kind: string } }> {
  ...
  if (input.kind === "article" && input.article) {
    ...
    const directCandidates = scored
      // DIRECT means same event or narrowly same issue, not merely same country/theme.
  ...
  const clean = deepSanitize(data);
  const audited = auditCitations(clean, evidence.length, baselines.length, ["headline", "points", "five_w", "so_what"]);
  const report = audited.report;
