...
3. Cite every factual statement with [n] for evidence items or [Bn] for structural baselines. One number per bracket: write [3][7], never [3, 7]. Cite only items that actually support the statement. Preserve attribution exactly: a source's verification caveat, allegation, estimate, denial, or assessment must not be reassigned to the actor being reported on.
...
- assessment: 2 to 4 sentences. The analytic judgment and its logic. Do not describe a ceasefire, treaty, agreement, institution, or diplomatic framework as "collapsed", "ended", "terminated", or "failed" unless the evidence explicitly establishes that status. Otherwise use bounded language such as "under severe strain", "eroding", or "implementation appears impaired".
...
- five_w: who, what, where, when, why, how. For article analysis, item [1] alone controls this section. Do not use [2+] or baselines to fill any 5W1H field. The "when" field must describe the event time only if item [1] states it. A publication timestamp is metadata, not event time. If item [1] does not state the event time, write "Not stated; item published YYYY-MM-DD" using item [1]'s publication date. Fill the other fields only with what [1] explicitly states or clearly supports. For "why", provide only an explicitly stated cause, motive, or rationale; do not restate the event itself as its cause. Write "Not stated" when [1] does not provide the element. Never write generic substitutes such as "Unspecified entities", "reported activities", or "unspecified locations".
- so_what: one or two specific sentences explaining the security, governance, humanitarian, or strategic implication supported by the evidence. Cite factual premises. Do not claim a peace agreement, ceasefire, institutional arrangement, or conflict-management framework has broken down or collapsed unless an evidence item explicitly establishes that status; otherwise use bounded language such as "places additional strain on". If the implication cannot be supported, say "Insufficient evidence to assess implications."
...
export function quickUser(args: { kind: "article" | "country" | "feed"; label: string; window: WindowKey; evidence: EvidenceItem[]; baselines: Baseline[]; metrics: EvidenceMetrics; ceiling: { band: string; reason: string }; statsDigest?: string; now: Date; directCount?: number }): string {
  const task =
    args.kind === "article"
      ? `Analyse item [1] as an analyst would on first read. Item [1] is the primary subject and must control the answer. Extract 5W1H only from [1]. Never use [2+] or baselines in 5W1H. Treat item [1]'s publication timestamp as publication metadata, not event time. Evidence items [1] through [${Math.max(1, args.directCount ?? 1)}] are DIRECT evidence. Items after that are CONTEXT only. DIRECT evidence may corroborate or contradict the selected item. CONTEXT may explain the broader environment but must not be described as corroboration and must not raise confidence. Ignore context that does not materially help interpretation. Never claim to have read the full article; you have the headline and excerpt only. Never output template or placeholder language.`
      : args.kind === "country"
